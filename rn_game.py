...
guess_this_number = randrange(0, 100)
...
try:
    for i in range(difficulty):
        ...
        elif guess < guess_this_number:
            print(f"The number is higher than {guess}")
        elif guess == guess_this_number:
            print(f"You guessed the correct number which was: {guess_this_number}.")
            break
        else:
            print(f"Wrong guess, the number was : {guess_this_number}")
except NameError:
    print("Difficulty level not set, please choose a valid difficulty level")